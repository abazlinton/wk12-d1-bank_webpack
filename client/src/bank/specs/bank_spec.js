...
describe('bank', function() {
  ...
  it("should be able to add interest", function(){
    var bank = new Bank();
    var account1 = new Account({owner:'Jay',amount:50, type:'business'});
    var account2 = new Account({owner:'Kieth',amount:150, type:'personal'});
    bank.addAccount(account1);
    bank.addAccount(account2);
    bank.addInterest(0.1);
    assert.equal(55, bank.findAccountByOwnerName('Jay').amount);
    assert.equal(165, bank.findAccountByOwnerName('Kieth').amount);
  });
});
